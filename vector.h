 ...
 * 
 * This structure maintains an array of up to MAX_VECTORS
 * named vectors and a count of how many are currently stored.
 * It replaces the need for global variables by grouping all
 * storage and management into one unit.
 */
typedef struct
{
    vector vectors[MAX_VECTORS]; /**< Array of stored vector objects. */
